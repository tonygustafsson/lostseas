...
export const validateJourney = (player?: Player, journeyLength = 0) => {
  ...
  if (!playerHasShips) errors.push("NO_SHIPS")
  if (shipsAreDamaged) errors.push("DAMAGED_SHIPS")
  if (player.crewMembers.count < minCrew) errors.push("NOT_ENOUGH_CREW_MEMBERS")
  if (player.crewMembers.count > maxCrew) errors.push("TOO_MANY_CREW_MEMBERS")
  if (crewIsAngry) errors.push("ANGRY_CREW")
  if (crewIsIll) errors.push("CREW_IS_ILL")
  if (!playerHasFood) errors.push("NO_FOOD")
  if (!playerHasWater) errors.push("NO_WATER")
